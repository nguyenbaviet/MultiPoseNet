...
class Tester(object):
    ...
    def test(self):
        ...
        import time
        # cap = cv2.VideoCapture(os.getcwd() + '/6people.avi')
        # cap = cv2.VideoCapture(os.getcwd() + '/infer_0705.avi')
        cap = cv2.VideoCapture(os.getcwd() + '/son1.avi')
        # cap = cv2.VideoCapture(os.getcwd() + '/13-05.avi')
        # cap = cv2.VideoCapture('/home/duong/lite-signage/1_cut.mkv')
        id = 0


        out = cv2.VideoWriter(os.getcwd() + '/test.avi', cv2.VideoWriter_fourcc(*'MJPG'), 30, (1920, 1080))
        start = time.time()
